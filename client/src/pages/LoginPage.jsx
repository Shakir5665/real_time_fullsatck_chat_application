...
const {login} = useContext(AuthContext); //  To connect with backend
...
const onSubmitHandler = (event) => {
    ...
    login(currentState === "Sign up" ? 'signup' : 'login' , {fullName , email , password, bio});  //  To connect with backend
}
  ...
  return (
    <div className='min-h-screen bg-cover bg-center flex items-center justify-center gap-8 sm:justify-evenly max-sm:flex-col backdrop-blur-2xl '>
        ...
        <form onSubmit={onSubmitHandler} className='border-2 bg-white/8 text-white border-gray-500 p-6 flex flex-col gap-6 rounded-lg shadow-lg'>
            ...
            <button type='submit' className='p-3 bg-gradient-to-r from-purple-400 to-violet-600 text-white rounded-md cursor-pointer'>
            ...
            <div className='flex flex-col gap-2'>

                {
                  currentState === "Sign up" ? (
                    <p className='text-small text-gray-600 flex gap-2'>Already have an account?  <span onClick={() => {setCurrentState('Login'); setIsDataSubmitted(false)}} className='font-medium text-violet-500 cursor-pointer'> Login here</span> </p> 
                    
                  ) : (
                    <p className='text-small text-gray-600 flex gap-2'>To Create an account<span onClick={() => {setCurrentState('Sign up')}} className='font-medium text-violet-500 cursor-pointer'>Click here</span></p>
                  )
                }
